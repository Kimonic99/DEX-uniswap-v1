...
export const getTokensAfterRemove = async (
  ...
  try {
    ...
    // Get the total supply of `Naija Dev` LP tokens
    ...
    // Similarly we also maintain a ratio for the `ND` tokens, so here in our case
    // Ratio is -> (amount of ND tokens sent back to the user / ND Token reserve) = (LP tokens withdrawn) / (total supply of LP tokens)
    // Then (amount of ND tokens sent back to the user) = (ND token reserve * LP tokens withdrawn) / (total supply of LP tokens)
    const _removeEther = _ethBalance.mul(removeLPTokenWei).div(_totalSupply);
    const _removeCD = naijaDevTokenReserve
